...
- ⚠️ **既存の `evals/baseline.json` (2026-08-14 10:41Z 取得) は `judge_coverage` を
  持たない**。当時のローカル実行が全 25 問を採点できていたのか、一部タイムアウトした
  残りの平均だったのかを事後に判別する手段がない。とくに
  `factual_correctness: 0.6248` は CI で全問タイムアウトした指標と同じものなので、
  カバレッジ検査を入れた状態でローカル取り直し (`--update-baseline`) を行い、
  独立した PR として baseline を更新するまでは、この基準値を信頼しないこと
