...
ヒットしなかった問いのうち、実際の回答内容を確認したものについては大半が
「同じ事実 (boto3 のサービス名等) を含む別の正しいチャンク」が検索されたことによる
もので、ADR 0007 に記載した原文書の構造的な重複によるノイズと一致する。
⚠️ ただしこの人手確認は上記の誤った 5 問リストに対して行ったため、
`bedrock-ug-014` / `025` / `028` は確認対象から漏れている。

`factual_correctness` が 1.0 に張り付いていない (0.625) ことは、judge が
